...
export class PoseManager {
  ...
  private async loadPoseCached(indexItem: PoseIndexItem): Promise<Pose> {
    const cached = this.poseCache.get(indexItem.id);
    if (cached) return cached;

    const pose = await loadPose(indexItem.file);
    this.poseCache.set(indexItem.id, pose);
    return pose;
  }
  ...
  private async preloadNext(fromId: string): Promise<void> {
    ...
    await Promise.all(
      preloadIds.map(async (id) => {
        if (this.poseCache.has(id)) return;
        const indexItem = store.poses.find((item) => item.id === id);
        if (!indexItem) return;
        try {
          const pose = await loadPose(indexItem.file);
          this.poseCache.set(id, pose);
        } catch {
